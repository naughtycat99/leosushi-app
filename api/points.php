...
function redeemPoints($input) {
    try {
        ...
        if ($customerPoints < $pointsRequired) {
            ...
            echo json_encode([
                'success' => false,
                'message' => "Nicht genug Punkte. Sie benötigen {$pointsRequired} Punkte, Sie haben derzeit {$customerPoints} Punkte."
            ]);
            return;
        }
        ...
        try {
            ...
            $description = "{$pointsRequired} Punkte gegen Gutscheincode eingetauscht";
            ...
            echo json_encode([
                ...
                'message' => "{$pointsRequired} Punkte erfolgreich in Gutscheincode eingetauscht: {$promoCode}"
            ]);
...
function createRedemptionRule($input) {
    try {
        ...
        if ($pointsRequired <= 0 || empty($discountType) || $discountValue <= 0) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'points_required, discount_type und discount_value sind erforderlich']);
            return;
        }
    ...
    try {
        ...
        try {
            ...
            echo json_encode([
                ...
                'valid_to' => $endDate,
                'message' => "Geburtstagsgutschein erstellt: {$promoCode}"
            ]);
