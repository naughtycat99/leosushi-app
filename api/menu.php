...
function listMenuItems() {
    try {
        ...
        // Removed filtering by available so frontend can show 'Sold out' status
        $isAdmin = isset($_GET['admin']) && $_GET['admin'] === 'true';
        
        if (!empty($branchId)) {
            $whereConditions[] = 'mi.branch_id = ?';
            $params[] = $branchId;
            $types .= 's';
        }
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Abrufen des Menüs: ' . $e->getMessage()
        ]);
    }
}
...
function getMenuItem($itemId) {
    try {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Abrufen des Gerichts: ' . $e->getMessage()
        ]);
    }
}
...
function createMenuItem($input) {
    try {
        ...
        if (empty($itemId) || empty($name) || empty($categoryId)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'item_id, name und category_id sind erforderlich']);
            return;
        }
        ...
        if ($checkStmt->get_result()->num_rows > 0) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'Gericht existiert bereits']);
            return;
        }
        ...
        $stmt = $conn->prepare('INSERT INTO menu_items (
            ...
            use_bullet_points, spicy, group_title, branch_id
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Erstellen des Gerichts: ' . $e->getMessage()
        ]);
    }
}
...
function updateMenuItem($itemId, $input) {
    try {
        if (empty($itemId)) {
            http_response_code(400);
            echo json_encode(['success' => false, 'message' => 'item_id ist erforderlich']);
            return;
        }
        ...
        if ($result->num_rows === 0) {
            http_response_code(404);
            echo json_encode(['success' => false, 'message' => 'Gericht existiert nicht']);
            return;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Gericht erfolgreich aktualisiert'
        ]);
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Aktualisieren des Gerichts: ' . $e->getMessage()
        ]);
    }
}
...
function deleteMenuItem($itemId) {
    try {
        ...
        echo json_encode([
            'success' => true,
            'message' => 'Gericht erfolgreich gelöscht'
        ]);
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Löschen des Gerichts: ' . $e->getMessage()
        ]);
    }
}
...
function listCategories() {
    try {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Abrufen der Kategorien: ' . $e->getMessage()
        ]);
    }
}
