...
function sendPromotionEmail($input) {
    try {
        $emails = $input['emails'] ?? [];
        $subject = $input['subject'] ?? 'Sonderangebot von LEO SUSHI';
        ...
        if (empty($emails) || !is_array($emails)) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'E-Mail-Liste ist erforderlich'
            ]);
            return;
        }
        ...
        echo json_encode([
            'success' => true,
            'message' => 'E-Mail an ' . count($results['sent']) . ' Kunden gesendet',
            'results' => $results
        ]);
...
function listCustomers() {
    try {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Fehler beim Abrufen der Kundenliste: ' . $e->getMessage()
        ]);
    }
}
