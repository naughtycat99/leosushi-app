...
function createReservation($input) {
    try {
        ...
        // Đảm bảo items là chuỗi JSON hợp lệ hoặc NULL
        $itemsJson = null;
        if (!empty($items)) {
        ...
        $stmt = $conn->prepare('
            INSERT INTO reservations (
                ...
                date, time, guests, table_number, note, items, customer_code, status, branch_id
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ');
        ...
        $stmt->bind_param('ssssssssiisssss', 
            ...
            $date, $time, $guests, $tableNumber, $note, $itemsJson, $customerCode, $status, $branchId
        );
...
function updateReservationStatus($input) {
    try {
        $reservationId = $input['reservation_id'] ?? $input['id'] ?? '';
        $status = $input['status'] ?? '';
        $reason = $input['reason'] ?? '';
        ...
        $conn = getDbConnection();
        $stmt = $conn->prepare('UPDATE reservations SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?');
        ...
        if ($stmt->affected_rows > 0) {
            ...
            if ($result->num_rows > 0) {
                ...
                $customerName = trim(($reservationData['first_name'] ?? '') . ' ' . ($reservationData['last_name'] ?? ''));

                if ($customerEmail) {
                    if ($status === 'confirmed') {
                        try {
                            sendReservationConfirmationEmail($customerEmail, $customerName, $reservationData);
                        } catch (Exception $e) {
                            error_log("Failed to send reservation confirmation email: " . $e->getMessage());
                        }
                    } elseif ($status === 'cancelled') {
                        try {
                            sendReservationCancellationEmail($customerEmail, $customerName, $reservationData, $reason);
                        } catch (Exception $e) {
                            error_log("Failed to send reservation cancellation email: " . $e->getMessage());
                        }
                    }
                }
            }
