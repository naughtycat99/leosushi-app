...
// Headers are handled by security-config.php setSecurityHeaders()
...
try {
    switch ($action) {
        ...
        case 'session': // Alias for v1/session
        case 'check':
            if ($method === 'GET') {
                checkAdminSession();
            } else {
        ...
        case 'end': // Alias for v1/session/end
        case 'logout':
        ...
        case 'restore': // Alias for v1/session/restore
        case 'restore-session':
            // Restore session from localStorage token (when cookie is lost after PWA restart)
            if ($method === 'POST') {
                restoreSessionFromToken($input);
            } else {
                http_response_code(405);
                echo json_encode(['success' => false, 'message' => 'Method not allowed']);
            }
        ...
        default:
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Action not found: ' . $action . '. Available: login, send-code, verify-code, check, logout, restore-session'
            ]);
    }
} catch (Exception $e) {
...
function adminLogin($input) {
    ...
    try {
        ...
        if ($checkTable->num_rows === 0) {
            ...
            if ($count == 0) {
                $defaultPassword = password_hash('Leo0301.', PASSWORD_BCRYPT);
                $conn->query("
                    INSERT INTO admin_users (id, username, password_hash, role) 
                    VALUES (1, 'admin', '$defaultPassword', 'owner')
                ");
            }
        }
    } catch (Exception $e) {
    ...
    if ($result->num_rows === 0) {
        ...
        $adminPassword = getenv('ADMIN_PASSWORD') ?: 'Leo0301.';
        ...
        if (password_verify($password, password_hash($adminPassword, PASSWORD_BCRYPT)) || $password === $adminPassword) {
            ...
            $_SESSION['admin_role'] = 'owner'; // Default role for legacy login
        ...
        if (password_verify($password, $admin['password_hash'])) {
            ...
            // Check if 2FA is enabled (email verification code)
            if (defined('ENABLE_2FA') && ENABLE_2FA) {
                ...
                $_SESSION['admin_role_temp'] = $admin['role'] ?? 'staff';
                
                echo json_encode([
                    'success' => true,
                    'requires_verification' => true,
                    'message' => 'Passwort korrekt. Bitte geben Sie den Bestätigungscode ein.',
                ...
                $updateStmt = $conn->prepare('
                    ...
                    WHERE id = ?
                ');
                $updateStmt->bind_param('ssi', $ip, $newSessionId, $admin['id']);
                $updateStmt->execute();
...
function checkAdminSession() {
    ...
    // Debug logging for login loop
    $sessionId = session_id();
    $adminLoggedIn = isset($_SESSION['admin_logged_in']) ? ($_SESSION['admin_logged_in'] ? 'true' : 'false') : 'not set';
    $adminSessionId = $_SESSION['admin_session_id'] ?? 'not set';
    
    // Check if logged in
    if (!isset($_SESSION['admin_logged_in']) || !$_SESSION['admin_logged_in']) {
        // Only log if we expect to be logged in (e.g. cookie exists) but session is empty
        if (isset($_COOKIE[session_name()])) {
            error_log("AdminAuth: Check failed - Not logged in. SessionID: $sessionId, admin_logged_in: $adminLoggedIn");
        }
    ...
    if (isset($_SESSION['admin_user_id']) && isset($_SESSION['admin_session_id'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($admin['current_session_id'] !== $_SESSION['admin_session_id']) {
                error_log("AdminAuth: Session mismatch! DB: {$admin['current_session_id']} vs Session: {$_SESSION['admin_session_id']}. Invalidating.");
                session_destroy();
                echo json_encode([
                    'success' => false, 
                    'logged_in' => false, 
...
/**
 * Restore admin session from a localStorage token.
 * When Android kills the PWA, PHP session cookies are lost.
 * The client stores the session token in localStorage (which persists).
 * This endpoint validates that token and re-creates the PHP session.
 */
function restoreSessionFromToken($input) {
    $token = $input['token'] ?? '';
    
    if (empty($token)) {
        echo json_encode(['success' => false, 'logged_in' => false, 'message' => 'No token provided']);
        return;
    }
    ...
    try {
        ...
        // Check if this token matches a valid admin session in the database
        $stmt = $conn->prepare('SELECT id, username, current_session_id FROM admin_users WHERE current_session_id = ? LIMIT 1');
        $stmt->bind_param('s', $token);
        $stmt->execute();
        $result = $stmt->get_result();
        
        if ($result->num_rows === 0) {
            echo json_encode(['success' => false, 'logged_in' => false, 'message' => 'Token invalid or expired']);
            return;
        }
        
        $admin = $result->fetch_assoc();
        
        // Token is valid! Restore the PHP session
        $_SESSION['admin_logged_in'] = true;
        $_SESSION['admin_login_time'] = time();
        ...
        error_log("AdminAuth: Session restored from token for user ID {$admin['id']}");
        ...
        error_log('AdminAuth: Token restore failed: ' . $e->getMessage());
        echo json_encode(['success' => false, 'logged_in' => false, 'message' => 'Server error']);
    }
}
...
function sendVerificationCode($input) {
    ...
    if ($result->num_rows === 0) {
        ...
        try {
            ...
            // Create default admin user (password: Leo0301.)
            $defaultPassword = password_hash('Leo0301.', PASSWORD_BCRYPT);
            ...
            if (!$adminExists) {
                ...
                // We don't force update here on every request to avoid overhead, 
                // but we will do it below if verification fails
            }
    ...
    if (!$hashValid || empty($admin['password_hash'])) {
        ...
        $newPasswordHash = password_hash('Leo0301.', PASSWORD_BCRYPT);
    ...
    // If password verification fails, try to reset password to default if it matches the new default
    // OR if it matches the legacy default (migrating from leo2024 to Leo0301.)
    if (!$passwordValid && ($password === 'Leo0301.' || $password === 'leo2024')) {
        error_log('Send verification code - Password match default/legacy but hash mismatch, updating password hash');
        $newPasswordHash = password_hash('Leo0301.', PASSWORD_BCRYPT);
        $updateStmt = $conn->prepare('UPDATE admin_users SET password_hash = ? WHERE id = ?');
        $updateStmt->bind_param('si', $newPasswordHash, $admin['id']);
        if ($updateStmt->execute()) {
            error_log('Send verification code - Password hash updated successfully');
            // Retry verification with new hash
            $passwordValid = password_verify($password, $newPasswordHash); // Will be true for Leo0301.
            if ($passwordValid || $password === 'Leo0301.') {
                $passwordValid = true;
                $admin['password_hash'] = $newPasswordHash;
            }
        } else {
            error_log('Send verification code - Failed to update password hash: ' . $updateStmt->error);
        }
    }
...
function verifyCodeAndLogin($input) {
    try {
        ...
        // Write session data and close session to prevent race conditions
        session_write_close();
