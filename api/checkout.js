...
function setupCheckoutPage() {
  ...
  if (scheduledDeliveryTimeGroup) {
    ...
    if (scheduledTimeEl && scheduledTimeEl.tagName === 'SELECT') {
      // Populate time dropdown with working hours (12:30 - 22:00) in 5-minute intervals
      const timeOptions = [];

      // Generate time slots from 12:30 to 22:00 in 5-minute intervals
      ...
      if (!scheduledTimeEl.value) {
        ...
        defaultMinutes = Math.ceil(defaultMinutes / 5) * 5;
...
async function confirmCheckout() {
  ...
  // Check if delivery address is within 4km range (Only for Delivery)
  if (isDelivery) {
    const rangeCheck = await checkDeliveryRange(street, postal, city);
    if (!rangeCheck.withinRange) {
      if (window.addNotification && window.NOTIFICATION_TYPES) {
  ...
  if (isDelivery && scheduledDateValue && scheduledTimeValue) {
    const now = new Date();
    // Parse date and time as local time (not UTC)
    ...
    if (scheduledDateTime < minDateTime) {
      ...
      if (window.addNotification && window.NOTIFICATION_TYPES) {
        window.addNotification(
          ...
          `Die gewünschte Lieferzeit muss mindestens 30 Minuten ab jetzt sein (frühestens ${minTimeStr}).`,
          { error: true }
        );
      } else {
  ...
  try {
    ...
    if (window.api && window.api.orders && window.api.orders.saveOrder) {
      ...
      const apiOrderData = {
        ...
        service_type: (window.selectedServiceType === 'pickup') ? 'Abholung' : 'Lieferung',
