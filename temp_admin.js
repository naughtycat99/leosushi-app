                ...
                const html = devices.length === 0
                    ...
                    : devices.map(ip => `
                        <div class="printer-item" style="display: flex; justify-content: space-between; align-items: center; padding: 10px; background: rgba(255,255,255,0.05); border-radius: 8px; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 4px;">
                            ...
                            <button onclick="event.stopPropagation(); connectNetworkPrinter('${ip}')" style="padding: 6px 12px; font-size: 11px; background: #10b981; border: none; border-radius: 4px; color: #fff; cursor: pointer; font-weight: 600;">Kết nối</button>
                        </div>
                    `).join('');
        ...
        function updatePrinterStatusUI() {
            ...
            // Set sound select value
            const soundSelect = document.getElementById('alarmSoundSelect');
            if (soundSelect) {
                soundSelect.value = localStorage.getItem('leo_alarm_sound_style') || 'phone_ring';
            }

            // Direct Sunmi V3 detection
            if (typeof SunmiNativePrinter !== 'undefined' && SunmiNativePrinter.hasPlugin()) {
                statusText.innerText = 'Máy in Sunmi V3 tích hợp (Sẵn sàng)';
        ...
        function changeAlarmSound(val) {
            localStorage.setItem('leo_alarm_sound_style', val);
            console.log('🔔 Alarm sound style changed to:', val);
            if (typeof playNotificationSound === 'function') {
                playNotificationSound();
            }
            if (typeof showMenuNotification === 'function') {
                showMenuNotification('🔔 Đã đổi kiểu chuông báo!', 'success');
            }
        }
            ...
            menu.innerHTML = `
                    ...
                    <div style="margin-bottom:12px;">
                        ...
                        <div style="display:flex;gap:6px;">
                            <button onclick="event.stopPropagation(); setPrinterType('bluetooth')" id="btnTypeBT" style="flex:1;padding:8px 6px;border-radius:8px;border:1px solid ${currentType === 'bluetooth' ? 'rgba(229,207,142,0.6)' : 'rgba(255,255,255,0.15)'};background:${currentType === 'bluetooth' ? 'rgba(229,207,142,0.15)' : 'transparent'};color:${currentType === 'bluetooth' ? '#e5cf8e' : 'rgba(255,255,255,0.5)'};cursor:pointer;font-size:12px;font-weight:600;transition:all 0.2s;">
                                📶 Bluetooth
                            </button>
                            <button onclick="event.stopPropagation(); setPrinterType('network')" id="btnTypeNet" style="flex:1;padding:8px 6px;border-radius:8px;border:1px solid ${currentType === 'network' ? 'rgba(16,185,129,0.6)' : 'rgba(255,255,255,0.15)'};background:${currentType === 'network' ? 'rgba(16,185,129,0.15)' : 'transparent'};color:${currentType === 'network' ? '#10b981' : 'rgba(255,255,255,0.5)'};cursor:pointer;font-size:12px;font-weight:600;transition:all 0.2s;">
                                🌐 WLAN/Netzwerk
                            </button>
                        </div>
                    </div>
                    ...
                    ${!connected ? `
                    <button onclick="event.stopPropagation(); pairPrinter()" style="width:100%;padding:10px;background:linear-gradient(180deg,#C2A355,#a8893e);color:#1a1a1a;border:none;border-radius:8px;font-weight:600;cursor:pointer;margin-bottom:8px;font-size:14px;">
                        🔍 Xuất hiện & Kết nối
                    </button>
                    ` : `
                    <button onclick="event.stopPropagation(); testPrint()" style="width:100%;padding:10px;background:rgba(16,185,129,0.2);color:#10b981;border:1px solid rgba(16,185,129,0.3);border-radius:8px;cursor:pointer;margin-bottom:8px;font-size:13px;">
                        📄 In thử
                    </button>
                    <button onclick="event.stopPropagation(); disconnectPrinter()" style="width:100%;padding:10px;background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.2);border-radius:8px;cursor:pointer;font-size:13px;">
                        ❌ Ngắt kết nối
                    </button>
                    `}
                    ` : `
                    ...
                    <div style="margin-bottom:12px; padding:10px; border-radius:8px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.1);">
                        ...
                        <div style="display:flex;gap:6px;">
                            ...
                            <button onclick="event.stopPropagation(); connectManualIpInMenu()" style="background:#10b981; color:#fff; border:none; padding:8px 12px; border-radius:6px; cursor:pointer; font-size:12px; font-weight:600;">Kết nối</button>
                        </div>
                    </div>
                    <button onclick="event.stopPropagation(); scanNetworkUIInMenu()" id="btnScanNet" style="width:100%;padding:10px;background:rgba(16,185,129,0.2);color:#10b981;border:1px solid rgba(16,185,129,0.3);border-radius:8px;cursor:pointer;margin-bottom:8px;font-size:13px;">
                        🔍 Quét tự động
                    </button>
                    <button onclick="event.stopPropagation(); testNetworkPrint()" style="width:100%;padding:10px;background:rgba(255,255,255,0.05);color:rgba(255,255,255,0.7);border:1px solid rgba(255,255,255,0.2);border-radius:8px;cursor:pointer;margin-bottom:8px;font-size:13px;">
                        📄 In thử (Hệ thống)
                    </button>
                    `}
                    
                    <div style="margin-top:12px; border-top:1px solid rgba(255,255,255,0.1); padding-top:12px;">
                        <button onclick="event.stopPropagation(); pingNativeBridge()" style="width:100%; padding:8px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.15); border-radius:6px; color:rgba(255,255,255,0.7); font-size:11px; cursor:pointer;">
                    ...
                    <div class="order-card" style="margin-bottom: 16px;">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <div style="display: flex; gap: 8px;">
                                <button class="btn-action btn-view" onclick="event.stopPropagation(); editDiscountCode('${code.promotion_id}')" title="Bearbeiten">✏️</button>
                                <button class="btn-action" onclick="event.stopPropagation(); deleteDiscountCode('${code.promotion_id}')" style="background: rgba(239,68,68,0.1);" title="Löschen">🗑️</button>
                            </div>
                        </div>
                    </div>
                `;
                ...
                <div class="time-schedule-content" style="max-width: 600px;" onclick="event.stopPropagation()">
                    ...
                    <form id="discountCodeForm" onsubmit="saveDiscountCode(event)">
                        ...
                        <div style="display: flex; gap: 12px;">
                            ...
                            <button type="button" class="btn-action" onclick="event.stopPropagation(); closeDiscountCodeModal()" style="background: rgba(239,68,68,0.1);">Abbrechen</button>
                        </div>
                    </form>
                </div>
            `;
                ...
                if (container) {
                    container.innerHTML = `
                        <div class="empty-state" style="background: rgba(239,68,68,.1); border: 2px solid rgba(239,68,68,.3);">
                            ...
                            <button class="btn-action btn-view" onclick="event.stopPropagation(); loadHolidaySchedule()" style="margin-top: 12px;">Erneut versuchen</button>
                        </div>
                    `;
                } else {
                ...
                return `
                    <div class="order-card" style="margin-bottom: 16px;">
                        <div style="display: flex; justify-content: space-between; align-items: start;">
                            ...
                            <div style="display: flex; gap: 8px;">
                                <button class="btn-action btn-view" onclick="event.stopPropagation(); editHolidaySchedule('${holiday.holiday_id}')" title="Bearbeiten">✏️</button>
                                <button class="btn-action" onclick="event.stopPropagation(); toggleHolidaySchedule('${holiday.holiday_id}')" style="background: ${holiday.is_active ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)'};" title="${holiday.is_active ? 'Deaktivieren' : 'Aktivieren'}">${holiday.is_active ? '⏸️' : '▶️'}</button>
                                <button class="btn-action" onclick="event.stopPropagation(); deleteHolidaySchedule('${holiday.holiday_id}')" style="background: rgba(239,68,68,0.1);" title="Löschen">🗑️</button>
                            </div>
                        </div>
                    </div>
                `;
                ...
                <div class="time-schedule-content" style="max-width: 600px;" onclick="event.stopPropagation()">
                    ...
                    <form id="holidayForm" onsubmit="saveHolidaySchedule(event)">
                        ...
                        <div style="display: flex; gap: 12px; justify-content: flex-end;">
                            <button type="button" class="btn-cancel-schedule" onclick="event.stopPropagation(); closeHolidayModal()">Abbrechen</button>
        ...
        function attachMenuItemEventListeners() {
            ...
            menuItemEventHandler = async function (e) {
                ...
                <div class="time-schedule-content" style="max-width: 600px; text-align: left;" onclick="event.stopPropagation()">
                    ...
                    <form id="menuItemForm" onsubmit="saveMenuItem(event)">
                        ...
                        <div style="display: flex; gap: 12px; margin-top: 30px;">
                            ...
                            <button type="button" class="btn-action" onclick="event.stopPropagation(); closeMenuItemModal()" style="padding: 14px 24px; background: rgba(239,68,68,0.15); border: 2px solid rgba(239,68,68,0.3); color: #ef4444; font-weight: 600; font-size: 15px; border-radius: 10px; cursor: pointer; transition: all 0.3s;">Abbrechen</button>
                        </div>
                    </form>
                </div>
            `;
            ...
            if (totalPages > 1) {
                paginationHTML = `
                    <div class="pagination" style="display: flex; justify-content: center; align-items: center; gap: 8px; margin-top: 24px; padding: 20px;">
                        <button class="filter-btn" onclick="event.stopPropagation(); goToCustomerPage(${currentCustomerPage - 1})" ${currentCustomerPage === 1 ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>← Zurück</button>
                        <span style="color: rgba(255, 255, 255, 0.7); padding: 0 16px;">
                            Seite ${currentCustomerPage} von ${totalPages} (${customers.length} Kunden)
                        </span>
                        <button class="filter-btn" onclick="event.stopPropagation(); goToCustomerPage(${currentCustomerPage + 1})" ${currentCustomerPage === totalPages ? 'disabled style="opacity: 0.5; cursor: not-allowed;"' : ''}>Weiter →</button>
                    </div>
                `;
            }
        ...
        function attachCustomerEventListeners() {
            ...
            customerEventHandler = function (e) {
                ...
                if (redeemBtn) {
                    ...
                    const customerName = redeemBtn.getAttribute('data-customer-name') || 'Khách hàng';
            ...
            modal.innerHTML = `
                <div class="time-schedule-content" style="max-width: 600px;" onclick="event.stopPropagation()">
                    ...
                    <form id="customerForm" onsubmit="saveCustomer(event)">
                        ...
                        <div style="display: flex; gap: 12px;">
                            <button type="submit" class="btn-action" style="flex: 1;">💾 Speichern</button>
                            <button type="button" class="btn-action" onclick="event.stopPropagation(); closeCustomerModal()" style="background: rgba(239,68,68,0.1);">Abbrechen</button>
                        </div>
                    </form>
                </div>
            `;
                ...
                <div class="time-schedule-content" style="max-width: 600px; text-align: left;" onclick="event.stopPropagation()">
                    ...
                    <form id="redeemPointsForm" onsubmit="redeemPointsForCustomer(event, '${customerId}', ${customerPoints})">
                        ...
                        <div style="display: flex; gap: 12px; margin-top: 30px;">
                            ...
                            <button type="button" class="btn-action" onclick="event.stopPropagation(); closeRedeemPointsModal()" style="padding: 14px 24px; background: rgba(239,68,68,0.15); border: 2px solid rgba(239,68,68,0.3); color: #ef4444; font-weight: 600; font-size: 15px; border-radius: 10px; cursor: pointer; transition: all 0.3s;">Abbrechen</button>
                        </div>
                    </form>
                </div>
            `;
                    ...
                    <div style="background: rgba(255, 255, 255, 0.03); border: 1px solid ${statusBorder}; border-radius: 12px; padding: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 16px;">
                            ...
                            <div style="display: flex; gap: 8px; align-items: center;">
                                ...
                                <button class="filter-btn" onclick="event.stopPropagation(); editRedemptionRule('${rule.rule_id}')" style="background: rgba(229, 207, 142, 0.1); padding: 8px 16px; font-size: 13px; cursor: pointer;">✏️ Bearbeiten</button>
                                <button class="filter-btn" onclick="event.stopPropagation(); deleteRedemptionRule('${rule.rule_id}')" style="background: rgba(239, 68, 68, 0.1); padding: 8px 16px; font-size: 13px; cursor: pointer;">🗑️ Löschen</button>
                            </div>
                        </div>
                    </div>
                `;
            ...
            modal.innerHTML = `
                <div class="time-schedule-content" style="max-width: 600px; text-align: left;" onclick="event.stopPropagation()">
                    ...
                    <form id="redemptionRuleForm" onsubmit="saveRedemptionRule(event, ${isEdit ? `'${rule.rule_id}'` : 'null'})">
                        ...
                        <div style="display: flex; gap: 12px; margin-top: 30px;">
                            <button type="submit" class="btn-action" style="flex: 1; padding: 14px 24px; background: linear-gradient(135deg, rgba(229,207,142,0.2), rgba(194,163,85,0.3)); border: 2px solid rgba(229,207,142,0.4); color: var(--gold); font-weight: 600; font-size: 15px; border-radius: 10px; cursor: pointer; transition: all 0.3s;">💾 Speichern</button>
                            <button type="button" class="btn-action" onclick="event.stopPropagation(); closeRedemptionRuleModal()" style="padding: 14px 24px; background: rgba(239,68,68,0.15); border: 2px solid rgba(239,68,68,0.3); color: #ef4444; font-weight: 600; font-size: 15px; border-radius: 10px; cursor: pointer; transition: all 0.3s;">Abbrechen</button>
                        </div>
                    </form>
                </div>
            `;
        ...
        function showNewOrderNotification(order) {
            ...
            if ('Notification' in window) {
                ...
                if (Notification.permission === 'granted') {
                    ...
                    const customerName = `${address.firstName || address.first_name || ''} ${address.lastName || address.last_name || ''}`.trim() || 'Khách hàng';
            ...
            if (scheduledTime && scheduledTime.time) {
                scheduledHTML = `
                        <div class="notification-order-item" style="background: rgba(251,191,36,0.15); padding: 6px; border-radius: 6px; border-left: 3px solid #fbbf24; margin: 4px 0;">
                            <span class="notification-order-label" style="color: #fbbf24; font-weight: bold;">Hẹn giờ:</span>
                            <span class="notification-order-value" style="color: #fbbf24; font-weight: bold;">${scheduledTime.time} (${scheduledTime.date || 'Hôm nay'})</span>
                        </div>
                `;
            }
            ...
            notification.innerHTML = `
                <div class="notification-header">
                    ...
                    <button class="notification-close" onclick="event.stopPropagation(); closeNewOrderNotification(this)">×</button>
                </div>
                <div class="notification-content">
                    ...
                    <div class="notification-actions">
                        <button class="notification-btn notification-btn-view" onclick="event.stopPropagation(); viewNewOrder('${orderId}'); closeNewOrderNotification(this.closest('.new-order-notification'))">Details anzeigen</button>
                        <button class="notification-btn notification-btn-confirm" onclick="event.stopPropagation(); confirmOrder('${orderId}'); closeNewOrderNotification(this.closest('.new-order-notification'))">Bestätigen</button>
                    </div>
                </div>
            `;
        ...
        function showNewReservationNotification(reservation) {
            ...
            if ('Notification' in window) {
                ...
                if (Notification.permission === 'granted') {
                    ...
                    const customerName = `${reservation.first_name || ''} ${reservation.last_name || ''}`.trim() || 'Khách hàng';
            ...
            notification.innerHTML = `
                ...
                <div class="notification-content">
                    ...
                    <div class="notification-actions">
                        <button class="notification-btn notification-btn-view" onclick="event.stopPropagation(); viewNewReservation('${reservationId}'); closeNewOrderNotification(this.closest('.new-order-notification'))">Details anzeigen</button>
                        <button class="notification-btn notification-btn-confirm" onclick="event.stopPropagation(); confirmReservation('${reservationId}'); closeNewOrderNotification(this.closest('.new-order-notification'))">Bestätigen</button>
                    </div>
                </div>
            `;
        ...
        function playSoundInternal() {
            try {
                ...
                const style = localStorage.getItem('leo_alarm_sound_style') || 'phone_ring';
                ...
                if (style === 'ding_dong') {
                    ...
                    osc2.type = 'sine';
                    osc2.frequency.setValueAtTime(523.25, audioCtx.currentTime + 0.35);
                    gain2.gain.setValueAtTime(0, audioCtx.currentTime + 0.35);
                    gain2.gain.linearRampToValueAtTime(0.4, audioCtx.currentTime + 0.37);
                    gain2.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 0.95);
                    osc2.start(audioCtx.currentTime + 0.35);
                    osc2.stop(audioCtx.currentTime + 0.95);
                    console.log('🔔 Played Ding-Dong alarm');
                } else if (style === 'phone_ring') {
                    const playRing = (startTime) => {
                        ...
                        for (let t = 0.03; t < 0.25; t += 0.03) {
                            osc.frequency.setValueAtTime(t % 0.06 < 0.03 ? 880 : 800, startTime + t);
                        }
                        gain.gain.setValueAtTime(0, startTime);
                        gain.gain.linearRampToValueAtTime(0.4, startTime + 0.02);
                        gain.gain.setValueAtTime(0.4, startTime + 0.23);
                        gain.gain.exponentialRampToValueAtTime(0.01, startTime + 0.25);
                        osc.start(startTime);
                        osc.stop(startTime + 0.25);
                    };
                    playRing(audioCtx.currentTime);
                    playRing(audioCtx.currentTime + 0.3);
                    console.log('🔔 Played Phone Ring alarm');
                } else if (style === 'siren') {
                    const osc = audioCtx.createOscillator();
                    const gain = audioCtx.createGain();
                    osc.connect(gain);
                    ...
                    osc.frequency.linearRampToValueAtTime(900, audioCtx.currentTime + 0.75);
                    ...
                    gain.gain.setValueAtTime(0.4, audioCtx.currentTime + 0.9);
                    gain.gain.exponentialRampToValueAtTime(0.01, audioCtx.currentTime + 1.0);
                    osc.start(audioCtx.currentTime);
                    osc.stop(audioCtx.currentTime + 1.0);
                    console.log('🔔 Played Siren alarm');
                } else {
                    // Default: double_beep
                    // Beep 1
                    const osc1 = audioCtx.createOscillator();
        ...
        // ---- Persistent alarm: repeat sound exactly 3 times for new orders ----
        let _alarmInterval = null;
        let _pendingAlarmOrders = 0;
        let _alarmPlayCount = 0;
        ...
        function checkPendingOrdersAndAlarm(orders) {
            ...
            // Only trigger alarm for orders we haven't played the alarm sound for yet
            const unalertedPending = pendingOrders.filter(o => {
                const id = o.order_id?.toString();
                return id && !alarmAlertedOrderIds.has(id);
            });

            if (unalertedPending.length > 0) {
                // Mark them as alerted immediately
                unalertedPending.forEach(o => {
                    alarmAlertedOrderIds.add(o.order_id.toString());
                });
                
                console.log('📢 Found unalerted pending orders, starting alarm...');
                startAlarm();
                startTitleBlink();
            } else if (pendingOrders.length === 0) {
                // If there are absolutely no pending orders left, make sure alarm is stopped
                stopAlarm();
                stopTitleBlink();
            }
        }
        ...
        async function printOrderBill(orderId, etaOverride = '') {
            ...
            const payMethod = pmStr.includes('cash') || pmStr.includes('tiền mặt') || pmStr.includes('bar') ? 'Tiền mặt' : pmStr.includes('paypal') ? 'PayPal' : (pmStr.includes('card') || pmStr.includes('karte') || pmStr.includes('thẻ') ? 'Thẻ' : 'Tiền mặt');
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  @media print {
    ...
    button { display: none; }
        ...
        .order-card:not(.expanded) .card-info,
        .order-card:not(.expanded) .order-timers,
        .order-card:not(.expanded) .card-actions {
...
<body>
  ...
  <table>
    ...
    ${discount > 0 ? `<tr><td>Giảm giá</td><td style="text-align:right;">-€${discount.toFixed(2)}</td></tr>` : ''}
  ...
  <div style="margin-top:16px; text-align:center;">
    ...
    <button onclick="window.close()" style="padding:8px 24px; font-size:14px; cursor:pointer; background:#666; color:#fff; border:none; border-radius:4px; margin-left:8px;">✕ Schließen</button>
        ...
        var isConfirmingOrder = false;
        var refreshPaused = false;
        var timerInterval = null;
        var alarmAlertedOrderIds = new Set();
        ...
        async function loadOrders(silent = false, preserveScroll = false) {
            ...
            const currentCounter = ++loadOrdersCounter;
            ...
            try {
                ...
                if (data.success && data.orders) {
                    ...
                    if (adminRoleObj && adminRoleObj.role === 'branch_admin' && adminRoleObj.branch) {
                        combinedData = combinedData.filter(o => {
                            let orderBranch = o.branch_id;
                            if (!orderBranch && o.summary) {
                                let sum = o.summary;
                                if (typeof sum === 'string') {
                                    try { sum = JSON.parse(sum); } catch (err) {}
                                }
                                if (sum && sum.branch && sum.branch.id) {
                                    orderBranch = sum.branch.id;
                                }
                            }
                            if (!orderBranch) orderBranch = 'branch_flora';
                            return orderBranch === adminRoleObj.branch;
                        });
                    }

                    if (currentCounter !== loadOrdersCounter) return;
                    allOrdersData = combinedData;
                    console.log('✅ Loaded data (merged + branch filtered):', allOrdersData.length);
                    window.__loadOrdersRunning = false;
                    ...
                    try {
                        ...
                        // checkAutoPrinting(realOrders); // Disabled per user request
                    } catch (autoPrintErr) {
                    ...
                    if (filteredOrders.length > 0) {
                        ...
                        ordersList.innerHTML = `
                            <div class="empty-state">
                                ...
                                <button class="btn-action" onclick="event.stopPropagation(); clearDatePicker(); loadOrders()" style="margin-top:15px;">Xem tất cả</button>
                            </div>
                        `;
                    }
                } else {
                ...
                if (!silent || (ordersList && ordersList.innerHTML.includes('Đang tải dữ liệu'))) {
                    ...
                    <div class="empty-state" style="background: rgba(239,68,68,.1); border: 2px solid rgba(239,68,68,.3);">
                        ...
                        <button class="btn-action btn-view" onclick="event.stopPropagation(); loadOrders()" style="margin-top: 12px;">Thử lại</button>
                    </div>
                    `;
                }
            }
        ...
        function applyDateFilterToOrders(orders) {
            // Primary filter: use the datePicker input (the date field visible in the UI)
            const datePicker = document.getElementById('datePicker');
            const selectedDate = datePicker ? datePicker.value : '';

            // If a specific date is picked, filter to that date only
            if (selectedDate) {
            ...
            // No date selected → show ALL orders (do not restrict to today)
            // Legacy fallback: support old dateFilter select element if it still exists
            const dateFilterEl = document.getElementById('dateFilter');
            const dateFilter = dateFilterEl ? dateFilterEl.value : 'all';

            if (dateFilter === 'all' || !dateFilterEl) {
                return orders;
        ...
        function applyDateFilter() {
            // Note: date filtering is handled by applyDateFilterToOrders via the datePicker input
            ...
            // Re-display orders with both date AND status filter
            if (allOrdersData && allOrdersData.length > 0) {
                let filteredOrders = applyDateFilterToOrders(allOrdersData);

                // Also apply the active status filter (e.g. "Đã xác nhận", "Chờ xử lý")
                const statusFilter = document.querySelector('#ordersContent .filter-btn.active[data-status]')?.dataset.status || 'all';
                if (statusFilter !== 'all') {
                    filteredOrders = filteredOrders.filter(o => (o.status || 'pending') === statusFilter);
                }
        ...
        function getStatusText(status) {
            ...
            return statusMap[status] || statusMap['pending'] || 'Chờ duyệt';
        }
        ...
        function renderOrderCard(order, forcedDate = null) {
            ...
            if (isReservation) {
                ...
                customerName = `${order.delivery_address?.first_name || order.delivery?.address?.firstName || ''} ${order.delivery_address?.last_name || order.delivery?.address?.lastName || ''} `.trim() || 'Khách hàng';
            ...
            return `
                ...
                <div class="order-card ${status}" data-order-id="${orderId}">
                    <div class="card-header-compact" onclick="const det = this.nextElementSibling; if(det.style.display==='none'){det.style.display='block';}else{det.style.display='none';}" style="display: flex; flex-direction: column; gap: 8px; cursor: pointer; padding: 8px 0;">
                        ...
                        ${ (status === 'completed' || status === 'delivered' || status === 'done') ? `
                    ...
                    <div class="card-info" style="display: none; margin-top:0; padding-top:12px; border-top:1px solid rgba(255,255,255,0.1);">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; padding-bottom: 12px; border-bottom: 1px solid rgba(255,255,255,0.1);">
                            <span style="font-size: 14px; font-weight: 500;">${itemCount} món</span>
                            <span style="background: ${(order.payment_status === 'paid' || summary.payment_status === 'paid') ? 'rgba(16,185,129,0.15)' : 'rgba(239,68,68,0.15)'}; color: ${(order.payment_status === 'paid' || summary.payment_status === 'paid') ? '#10b981' : '#ef4444'}; padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600;">
                                ${(order.payment_status === 'paid' || summary.payment_status === 'paid') ? '✅ Đã thanh toán' : '❌ Chưa thanh toán'}
                            </span>
                        </div>
                    ...
                    <div class="card-actions" style="margin-top: 16px;">
                        ${status === 'pending' ? `
                            <button class="btn-action btn-confirm" onclick="event.stopPropagation(); ${isReservation ? `confirmReservation('${orderId}')` : `confirmOrder('${orderId}')`}" style="background: #f59e0b; color: #fff; font-size: 18px; font-weight: 700; padding: 12px; border-radius: 8px; width: 100%; border: none;">Duyệt đơn</button>
                            <button class="btn-action btn-cancel" onclick="event.stopPropagation(); ${isReservation ? `cancelReservation('${orderId}')` : `cancelOrder('${orderId}')`}" style="margin-top: 8px; background: transparent; border: 1px solid rgba(255,255,255,0.2); color: rgba(255,255,255,0.6); padding: 10px; border-radius: 8px; width: 100%;">Hủy đơn</button>
                        ` : ''}
                        
                        ${(status === 'confirmed' || status === 'in_delivery') && !isReservation ? `
                            <button class="btn-action btn-confirm" onclick="event.stopPropagation(); completeOrder('${orderId}')" style="background: #10b981; color: #fff; font-size: 18px; font-weight: 700; padding: 12px; border-radius: 8px; width: 100%; border: none;">✓ Hoàn thành</button>
                        ` : ''}
                        
                        ${(status !== 'pending' && !isReservation) ? `
                            <button class="btn-action" onclick="event.stopPropagation(); printOrderBill('${orderId}')" style="margin-top: 12px; width: 100%; padding: 12px; border: 1px dashed rgba(255,255,255,0.2); background: transparent; color: rgba(255,255,255,0.8); border-radius: 8px; transition: all 0.2s ease;">🖨️ In Hóa Đơn</button>
                        ` : ''}
        ...
        function displayOrders(orders) {
            ...
            if (!orders || orders.length === 0) {
                ...
                <div class="empty-state">
                        ...
                        <button onclick="event.stopPropagation(); loadOrders()" style="margin-top:15px; padding:8px 20px; background:var(--gold); border:none; border-radius:5px; cursor:pointer; color:#000; font-weight:bold;">🔄 Thử tải lại</button>
                    </div>
                `;
                return;
            }
            ...
            ">
                <span>📅 ${formattedDate}</span>
                    ${orderDate === today ? '<span style="font-size: 12px; background: var(--gold); color: #000; padding: 2px 8px; border-radius: 10px; font-weight: bold;">HÔM NAY</span>' : ''}
                </div>
                `;
                    lastDate = orderDate;
                }
        ...
        async function loadReservations(preserveScroll = false) {
            ...
            try {
                ...
                <div class="empty-state" style="background: rgba(239,68,68,.1); border: 2px solid rgba(239,68,68,.3);">
                                ...
                                <p style="color: rgba(255,255,255,.7); margin-bottom: 12px;">${error.message || 'Unknown error'}</p>
                                <button class="btn-action btn-view" onclick="event.stopPropagation(); loadReservations()" style="margin-top: 12px;">Erneut versuchen</button>
                            </div>
                `;
            }
        }
            ...
            return `
                <div class="reservation-card ${status}" data-reservation-id="${reservationId}">
                    ...
                    <div class="card-actions">
                        ${status === 'pending' ? `
                            <button class="btn-action btn-confirm" onclick="event.stopPropagation(); confirmReservation('${reservationId}')">✓ Xác nhận</button>
                            <button class="btn-action btn-cancel" onclick="event.stopPropagation(); cancelReservation('${reservationId}')">✗ Hủy</button>
                        ` : ''}
                    </div>
                </div>
                `;
        ...
        function displayReservations(reservations, targetElement = null) {
            ...
            ">
                <span>📅 ${formattedDate}</span>
                    ${resDate === today ? '<span style="font-size: 12px; background: var(--gold); color: #000; padding: 2px 8px; border-radius: 10px; font-weight: bold;">HÔM NAY</span>' : ''}
                            </div>
                `;
                    lastDate = resDate;
                }
        ...
        async function confirmReservation(reservationId) {
            ...
            if (reservationForEmail.email) {
                if (typeof sendReservationConfirmationEmail === 'function') {
                    ...
                    try {
                        if (typeof emailjs !== 'undefined' && typeof EMAILJS_CONFIG !== 'undefined') {
                            const customerName = `${reservationForEmail.firstName} ${reservationForEmail.lastName}`.trim() || 'Khách hàng';
            ...
            try {
                ...
                if (data.success) {
                    ...
                    if (reservation && reservation.email) {
                        try {
                            if (typeof emailjs !== 'undefined' && typeof EMAILJS_CONFIG !== 'undefined') {
                                const customerName = `${reservation.first_name || reservation.firstName || ''} ${reservation.last_name || reservation.lastName || ''}`.trim() || 'Khách hàng';
                                const customerTime = reservation.time || '';
                                let formattedTime = '';
                                if (customerTime) {
                                    const timeParts = customerTime.split(':');
        ...
        async function viewOrderDetails(orderId) {
            ...
            const payMethod = pmStr.includes('cash') || pmStr.includes('tiền mặt') || pmStr.includes('bar') ? 'Tiền mặt' : pmStr.includes('paypal') ? 'PayPal' : (pmStr.includes('card') || pmStr.includes('karte') || pmStr.includes('thẻ') ? 'Thẻ' : 'Tiền mặt');
            const status = order.status || 'pending';
            const statusText = { 'pending': 'Chờ duyệt', 'confirmed': 'Đang làm', 'cancelled': 'Đã hủy', 'completed': 'Đã xong' }[status] || 'Chờ duyệt';
        ...
        <div
            ...
            <div style="display:flex;justify-content:space-between;margin-bottom:6px;"><span
                    style="color:rgba(255,255,255,0.6);">Tạm tính</span><span
            ...
            ${parseFloat(summary.discount || 0) > 0 ? `<div
                style="display:flex;justify-content:space-between;margin-bottom:6px;"><span
                    style="color:rgba(255,255,255,0.6);">Giảm giá</span><span
        ...
        <button class="admin-success-btn" onclick="event.stopPropagation(); closeAdminSuccessNotification()">Verstanden</button>
    </div>
                `;
        ...
        <button class="admin-success-btn" onclick="event.stopPropagation(); closeAdminReservationSuccessNotification()">Verstanden</button>
    </div>
                `;
        ...
        if (!window._adminPollingInterval) {
            window._adminPollingInterval = setInterval(() => {
                if (window.__loadOrdersRunning || typeof loadOrders !== 'function') return;
