...
// Finding the slice
const startHtml = content.indexOf('return `\\n                <div class="order-card ${status}" data-order-id="${orderId}">');
if (startHtml !== -1) {
    ...
    } else { console.log('endHtmlTarget not found'); }
} else { console.log('startHtml not found'); }

// Preventing click bubbling
content = content.replace(/onclick="([a-zA-Z0-9_]+)\(/g, 'onclick="event.stopPropagation(); $1(');
