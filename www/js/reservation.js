...
// Get table status for a specific date and time (from MySQL API)
...
/**
 * Setup time options for reservation dropdown
 * Returns true if at least one slot is available for the selected date
 */
function setupReservationTimeOptions() {
  ...
  for (let hour = openHour; hour < closeHour; hour++) {
    for (let minute = 0; minute < 60; minute += 5) {
      ...
      if (selectedDate === today) {
        ...
        const slotTime = new Date(`${today}T${timeStr}`);
  ...
  // Highlight available slots for user
  if (timeOptions.length === 0) {
    const option = document.createElement('option');
    option.value = "";
    option.textContent = "Keine Termine verfügbar";
    option.disabled = true;
    timeSelect.appendChild(option);
  }

  // Set default selection if none exists or if it's currently empty
  if ((!timeSelect.value || timeSelect.value === "") && timeOptions.length > 0) {
...
/**
 * Initialize reservation date with rollover logic
 * If today is near/past closing, select tomorrow
 */
function initReservationDateTime() {
  console.log('🔄 initReservationDateTime called');
  const dateInput = document.getElementById('reserveDate');
  if (!dateInput) return;
  ...
  const hasSlotsToday = setupReservationTimeOptions();
...
function setupTableSelectionListeners() {
  ...
  if (dateInput) {
    dateInput.addEventListener('change', () => {
        setupReservationTimeOptions();
        updateTableSelection();
    });
  }

  if (timeSelect) {
    timeSelect.addEventListener('change', updateTableSelection);
  }
}

// Setup reservation form
function setupReservationForm() {
  initReservationDateTime();
...
async function handleReservation(event) {
  ...
  if (window.addNotification && window.NOTIFICATION_TYPES) {
    window.addNotification(
      ...
      `Ihre Reservierung für ${reservationData.guests} Personen am ${reservationData.date} um ${reservationData.time} wurde erfolgreich gesendet.`,
...
window.initReservationDateTime = initReservationDateTime;
