    ...
    return `
      <div style="display: flex; justify-content: space-between; align-items: center; padding: 8px; margin-bottom: 8px; background: rgba(255,255,255,.05); border-radius: 6px;">
        <div style="flex: 1;">
          ...
          <div style="font-size: 12px; color: rgba(255,255,255,.6);">Anzahl: ${item.qty} x ${item.price} €</div>
        </div>
        <div style="text-align: right; margin-left: 10px;">
          <div style="font-weight: 600; color: var(--gold);">€${itemTotal.toFixed(2)}</div>
          <button onclick="removeFromReservationCart(${index})" style="margin-top: 4px; padding: 4px 8px; background: rgba(239,68,68,.2); border: 1px solid rgba(239,68,68,.3); color: #ef4444; border-radius: 4px; cursor: pointer; font-size: 11px;">Löschen</button>
        </div>
      </div>
    `;
  }).join('') + `
...
// Check if delivery address is within delivery range (5km)
function checkDeliveryRange(street, postal, city) {
  ...
  if (valid5kmZips.includes(customerPostal)) {
    return {
      withinRange: true,
      distance: null,
      message: '✓ Lieferung möglich (innerhalb 5km - kostenlos)'
    };
  }
...
function renderPayPalButton() {
  ...
  if (total <= 0) {
    container.innerHTML = '<p style="color: #ef4444; padding: 10px;">Fehler: Gesamtbetrag ist 0. Bitte fügen Sie Artikel hinzu.</p>';
    return;
  }
  ...
  paypal.Buttons({
    ...
    onApprove: async function (data, actions) {
      ...
      return actions.order.capture().then(async function (details) {
        ...
        // Save order to Firebase / MySQL
        const apiResult = await saveOrderToFirebase(orderData);
...
function selectPaymentOption(method) {
  ...
  if (confirmPaymentBtn) {
    confirmPaymentBtn.style.display = (method === 'paypal' || method === 'card') ? 'none' : 'block';
  }
  ...
  if (method === 'cash' && cashOption) {
    ...
    cardOption.classList.add('selected');
    // Show PayPal container but render CARD button (no PayPal account needed)
    if (paypalButtonContainer) {
...
// Render PayPal CARD button (Visa/Mastercard - no PayPal account needed)
function renderPayPalCardButton() {
  ...
  if (typeof paypal === 'undefined') {
    container.innerHTML = '<p style="color:#ef4444;padding:10px;">PayPal SDK nicht geladen. Bitte Seite neu laden.</p>';
    return;
  }
  ...
  if (selectedServiceType === 'delivery') {
    ...
    const postal = document.getElementById('deliveryPostal')?.value;
    const city = document.getElementById('deliveryCity')?.value;
    if (street && postal && city) deliveryFee = 2.50;
  ...
  paypal.Buttons({
    fundingSource: paypal.FUNDING.CARD, // Force card payment UI (no PayPal login)
    style: {
      layout: 'vertical',
      shape: 'rect',
      label: 'pay'
      ...
      return actions.order.capture().then(async function (details) {
        ...
        if (deliveryAddress.email && deliveryAddress.phone) {
          try {
            ...
            if (savedCustomerInfo?.customerCode) {
              deliveryAddress.customerCode = savedCustomerInfo.customerCode;
            }
          } catch (e) { console.error('Error saving customer info:', e); }
        }
        ...
        const orderData = {
          ...
          summary: {
            ...
            delivery_fee: deliveryFee.toFixed(2),
            total: total.toFixed(2),
            payment_method: 'card',  // saved as card (paid online via PayPal card)
            payment_status: 'paid',
            timestamp: orderTimestamp,
            paypal_order_id: details.id
          },
          customerCode: deliveryAddress.customerCode || savedCustomerInfo?.customerCode || null,
          createdAt: orderTimestamp
        };
    ...
    onError: function (err) {
      ...
      alert('Ein Fehler bei der Kartenzahlung. Bitte erneut versuchen.');
    },
    onCancel: function () {
...
async function confirmPayment() {
  ...
  let scheduledDeliveryTime = typeof getScheduledDeliveryTime === 'function' ? getScheduledDeliveryTime() : null;

  // Auto-convert ASAP orders placed while closed into Scheduled Orders
  if (!scheduledDeliveryTime && typeof isRestaurantOpen === 'function' && !isRestaurantOpen()) {
    let now = new Date();
    let berlinTimeStr = now.toLocaleString('en-US', { timeZone: 'Europe/Berlin', hour12: false });
    let berlinDate = new Date(berlinTimeStr);
    let hour = berlinDate.getHours();
    
    let targetDate = new Date(berlinDate);
    let openHour = (typeof RESTAURANT_HOURS !== 'undefined') ? RESTAURANT_HOURS.open : 12;
    let closeHour = (typeof RESTAURANT_HOURS !== 'undefined') ? RESTAURANT_HOURS.close : 22;
    
    let isNextDay = false;
    // If we're past closing time or at night before midnight, it's for 'tomorrow' or 'next opening'
    if (hour >= closeHour) {
      targetDate.setDate(targetDate.getDate() + 1);
      isNextDay = true;
    }
    
    let dateString = new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Berlin', year: 'numeric', month: '2-digit', day: '2-digit' }).format(targetDate);
    
    let bufferMinutes = 0; // Deliver exactly at opening time
    let finalHour = openHour + Math.floor(bufferMinutes / 60);
    let finalMin = bufferMinutes % 60;
    let timeString = `${String(finalHour).padStart(2, '0')}:${String(finalMin).padStart(2, '0')}`;
    ...
    let actionWord = (selectedServiceType === 'delivery') ? 'Lieferung' : 'Abholung';
    let dayMsg = isNextDay ? `morgige ${actionWord}` : `heutige ${actionWord}`;
    alert(`Hinweis: Das Restaurant ist zurzeit geschlossen.\nDa Sie keine Wunschzeit gewählt haben, wurde Ihre Bestellung automatisch für die ${dayMsg} um ${timeString} Uhr vorgemerkt.`);
  }
...
function saveOrderToDailyReport(orderData) {
  ...
  // Also send formatted email notification to OWNER (Admin)
  // sendOrderDataToAdmin(orderData); // Disable raw JSON email
  // Send beautiful email to owner
  const deliveryAddress = orderData.delivery?.address || orderData.delivery_address || {};
  if (typeof sendOrderEmail === 'function') {
    console.log('📧 Triggering new order email to owner...');
    sendOrderEmail(orderData, deliveryAddress);
  }
}
...
function sendCustomerConfirmationEmail(orderData, deliveryAddress, isAdminConfirmed = false, customEstimatedTime = null) {
  ...
  const templateParams = {
    ...
    restaurant_address: (orderData.branch_id === 'branch_haupt' || orderData.branchId === 'branch_haupt' || (orderData.summary && orderData.summary.branch && orderData.summary.branch.id === 'branch_haupt') || (orderData.branch && orderData.branch.id === 'branch_haupt')) ? 'Hauptstraße 29a, 13158 Berlin' : 'Florastraße 10A, 13187 Berlin',
    restaurant_phone: (orderData.branch_id === 'branch_haupt' || orderData.branchId === 'branch_haupt' || (orderData.summary && orderData.summary.branch && orderData.summary.branch.id === 'branch_haupt') || (orderData.branch && orderData.branch.id === 'branch_haupt')) ? '030 63912199' : '030 71055810',
...
function sendCancellationEmail(orderData) {
  if (typeof emailjs === 'undefined') {
    console.warn('EmailJS not loaded. Please configure EmailJS in script.js');
    return;
  }
  ...
  let deliveryAddress = orderData.delivery?.address || orderData.delivery_address || {};
  ...
  // Backup email check
  const customerEmail = deliveryAddress.email || orderData.email || orderData.delivery?.email || '';

  if (!customerEmail || customerEmail.trim() === '') {
    console.warn('Customer email not provided for cancellation email');
    return;
  }

  // Ensure address object has email
  if (!deliveryAddress.email) {
    deliveryAddress.email = customerEmail;
  }
  ...
  const orderId = orderData.order_id || `ORD-${Date.now()}`;
  const orderTime = new Date().toLocaleString('de-DE');

  // Cancellation Message
  const cancellationMessage = 'Ihre Bestellung wurde leider storniert. Falls Sie bereits bezahlt haben, wird der Betrag erstattet. Bitte kontaktieren Sie uns für weitere Informationen.';
  ...
  const templateParams = {
    ...
    order_id: orderId,
    order_time: orderTime,
    estimated_time: 'Storniert',
    is_confirmed: 'Storniert',
    confirmation_message: cancellationMessage,
    items: itemsList,
    item_count: orderData.summary?.item_count || 0,
    subtotal: `€${orderData.summary?.subtotal || '0.00'}`,
    delivery_fee: `€${orderData.summary?.delivery_fee || '0.00'}`,
  ...
  emailjs.send(
    getEmailjsConfig().SERVICE_ID,
    getEmailjsConfig().CUSTOMER_TEMPLATE_ID || getEmailjsConfig().TEMPLATE_ID,
    templateParams
  )
    .then((response) => {
...
function showPrintBills(orderData, deliveryAddress, orderId, estimatedTimeText = '') {
  // ---- TRY BLUETOOTH AUTO-PRINT FIRST ----
  if (typeof BluetoothPrinter !== 'undefined' && BluetoothPrinter.isConnected) {
    console.log('🖨️ Bluetooth printer connected — auto-printing...');
    BluetoothPrinter.printReceipt(orderData, deliveryAddress, orderId, estimatedTimeText)
      .then(() => {
        console.log('🖨️ ✅ Auto-printed via Bluetooth!');
        if (typeof showMenuNotification === 'function') {
          showMenuNotification('🖨️ Rechnung automatisch gedruckt!', 'success');
        }
      })
      .catch(err => {
        console.error('🖨️ Bluetooth print failed, falling back to browser:', err);
        if (typeof showMenuNotification === 'function') {
          showMenuNotification('⚠️ Bluetooth-Druck fehlgeschlagen. Browser-Druck wird verwendet.', 'warning');
        }
        // Fall back to browser print
        browserPrint(orderData, deliveryAddress, orderId, estimatedTimeText);
      });
    return; // Done — don't open browser print
  }

  // ---- FALLBACK: BROWSER PRINT (hidden iframe) ----
  browserPrint(orderData, deliveryAddress, orderId, estimatedTimeText);
}

function browserPrint(orderData, deliveryAddress, orderId, estimatedTimeText) {
  ...
  // Create a hidden iframe instead of window.open to bypass popup blockers
  ...
  // Clean up iframe after printing is presumed done
  setTimeout(() => {
...
function generateCustomerReceipt(orderData, deliveryAddress, orderId, estimatedTimeText = '') {
  const isHauptBranch = (orderData.branch_id === 'branch_haupt' || orderData.branchId === 'branch_haupt' || (orderData.summary && orderData.summary.branch && orderData.summary.branch.id === 'branch_haupt') || (orderData.branch && orderData.branch.id === 'branch_haupt'));
  const now = new Date();
    ...
    <div class="bill-page">
      ...
      <div class="bill-header">
        ...
        <div class="restaurant-info">
          ${isHauptBranch ? 'Hauptstraße 29a<br>13158 Berlin' : 'Florastraße 10A<br>13187 Berlin'}<br>
          ${isHauptBranch ? '03063912199' : '03071055810'}
        </div>
      </div>
      ...
      <div class="bill-section">
        ...
        <div class="customer-info">
          ...
          ${(() => {
            ...
            const hn = deliveryAddress.houseNumber || deliveryAddress.house_number || deliveryAddress.housenumber || '';
            const streetLine = (hn && !street.includes(hn)) ? `${street} ${hn}` : street;
            return orderData.service_type === 'delivery' ? `<br>Adresse: ${streetLine}, ${deliveryAddress.postal || ''} ${deliveryAddress.city || ''}` : '';
          })()}
        </div>
      </div>
...
function sendReservationEmail(reservationData) {
  ...
  const templateParams = {
    ...
    restaurant_name: 'LEO SUSHI',
    restaurant_address: (reservationData.branch_id === 'branch_haupt' || reservationData.branchId === 'branch_haupt' || (reservationData.branch && reservationData.branch.id === 'branch_haupt')) ? 'Hauptstraße 29a, 13158 Berlin' : 'Florastraße 10A, 13187 Berlin',
    restaurant_phone: (reservationData.branch_id === 'branch_haupt' || reservationData.branchId === 'branch_haupt' || (reservationData.branch && reservationData.branch.id === 'branch_haupt')) ? '030 63912199' : '030 71055810'
  };
