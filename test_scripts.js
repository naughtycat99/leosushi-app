...
<body>
  ...
  <div style="margin-top:16px; text-align:center;">
    ...
    <button onclick="window.close()" style="padding:8px 24px; font-size:14px; cursor:pointer; background:#666; color:#fff; border:none; border-radius:4px; margin-left:8px;">✕ Schließen</button>
        ...
        // Check if modal is open - MOVED TO TOP to avoid ReferenceError
        ...
        // Request permission for push notifications - MOVED TO TOP 
        function requestNotificationPermission() {
            if (!('Notification' in window)) {
                ...
                Notification.requestPermission().then(permission => {
                    if (permission === 'granted') {
                        ...
                        if (typeof showMenuNotification === 'function') {
                            showMenuNotification('Thông báo đã được bật!', 'success');
                        }
                    }
                });
            }
        ...
        // Load statistics
        // Statistics handled by js/admin-stats.js
        ...
        async function loadOrders(silent = false, preserveScroll = false) {
            ...
            try {
                ...
                if (window.api && window.api.orders && window.api.orders.list) {
                    ...
                    // Mock fetch response structure for compatibility with existing code
                    response = {
                ...
                if (data.success && data.orders) {
                        ...
                        try {
                            ...
                            if (resResponse.ok) {
                                ...
                                if (resData.success && resData.reservations) {
                                    // Tag reservations and add to combined list
                            ...
                            console.warn('⚠️ Could not merge reservations:', resErr);
                        }
                    }
                    ...
                    try {
                        checkAutoPrinting(allOrdersData.filter(o => !o.is_merged_reservation)); // Only auto-print real orders
                    } catch (autoPrintErr) {
                    ...
                    if (filteredOrders.length > 0) {
                        if (preserveScroll) {
                            // Smart update mode: chỉ update thay đổi
                ...
                <div class="empty-state">
                                <div class="empty-state-icon">📦</div>
                                <h3 style="color: rgba(255,255,255,.8); margin: 16px 0 8px; font-size: 18px;">Không có đơn hàng</h3>
                ...
                <div class="empty-state" style="background: rgba(239,68,68,.1); border: 2px solid rgba(239,68,68,.3);">
                                <div class="empty-state-icon">❌</div>
                                <h3 style="color: #ef4444; margin: 16px 0 8px; font-size: 18px;">Lỗi tải đơn hàng</h3>
        ...
        function smartUpdateOrders(newOrders) {
            ...
            // Update changed orders — Re-render full card for status changes (so timer container appears)
            updatedOrders.forEach(order => {
                const orderId = (order.order_id || order._id || '').toString();
                const existingCard = ordersList.querySelector(`[data-order-id="${orderId}"]`);
                if (existingCard) {
                    // Full re-render so timer div and action buttons update correctly
            ...
            // Re-init timers after any card changes (additions or status updates)
        ...
        function renderOrderCard(order, forcedDate = null) {
            ...
            const orderId = order.order_id || (summary.timestamp ? summary.timestamp : order.id) || 'N/A';
            const orderIdShort = summary.short_id || (orderId.toString().includes('-') ? 'LEO-' + orderId.toString().split('-').pop() : orderId.toString().slice(-8));
        ...
        function displayOrders(orders) {
            ...
            if (!orders || orders.length === 0) {
                ...
                <div class="empty-state">
                        <div class="empty-state-icon">📦</div>
                        <h3 style="color: rgba(255,255,255,.8); margin: 16px 0 8px; font-size: 18px;">Keine Bestellungen gefunden</h3>
                        <p style="color: rgba(255,255,255,.5);">Mới nhất sẽ xuất hiện ở đây</p>
                    </div>
                `;
                return;
            }
        ...
        async function checkAdminLogin() {
            ...
            try {
                ...
                if (isLoggedIn) {
                    ...
                    if (sidebarStatsBtn) {
                        sidebarStatsBtn.style.display = (role === 'owner') ? 'flex' : 'none';
                    }
                } else {
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            // 1. Initialize printer and pickers
            ...
            if (isLoggedIn) {
                try {
                    ...
                    if (typeof loadDiscountCodes === 'function') await loadDiscountCodes();
                } catch (error) {
            ...
            // 9. Auto-refresh loops
            setInterval(() => {
                if (localStorage.getItem('leo_admin_logged_in') !== 'true' || refreshPaused) return;

                if (!isUserInteracting && !isModalOpen()) {
                    const activeTab = document.querySelector('.admin-tab.active');
                    const tabText = activeTab ? activeTab.textContent.trim() : '';

                    if (tabText.includes('Bestellungen')) {
                        loadOrders(true, true);
