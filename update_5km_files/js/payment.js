...
async function renderPayPalButton() {
  ...
  // Service fee (default 0 or from elsewhere)
  const serviceFee = window.serviceFee || 0;
  ...
  try {
    paypal.Buttons({
      ...
      onApprove: async function (data, actions) {
        ...
        // ❗ CHƯ A set cờ ở đây - chỉ set SAU KHI capture() thành công
        ...
        return actions.order.capture().then(async function (details) {
          ...
          // ✅ Chỉ set cờ SAU KHI capture() trả về thành công (status = COMPLETED)
          if (details.status !== 'COMPLETED') {
            console.error('❌ [PayPal onApprove] Capture status không phải COMPLETED:', details.status);
            window._paypalPaymentCompleted = false;
            alert('⚠️ Thanh toán PayPal chưa được xác nhận hoàn toàn. Vui lòng liên hệ nhà hàng nếu tiền đã bị trừ.');
            return;
          }
          ...
          const apiOrderData = {
            ...
            service_type: (selectedServiceType === 'pickup') ? 'Abholung' : 'Lieferung',
            payment_method: 'PayPal',
          ...
          if (window.api && window.api.orders && window.api.orders.saveOrder) {
            ...
            try {
              const apiResult = await window.api.orders.saveOrder(apiOrderData);
          ...
          if (window.addNotification && window.NOTIFICATION_TYPES) {
            window.addNotification(
              ...
              'Die PayPal-Zahlung konnte nicht verarbeitet werden. Ihre Bestellung wurde nicht aufgegeben. Bitte versuchen Sie es erneut oder wählen Sie eine andere Zahlungsmethode.',
              { error: true }
            );
          } else {
            alert('❌ Bestellung fehlgeschlagen!\n\nDie PayPal-Zahlung konnte nicht verarbeitet werden.\nIhre Bestellung wurde nicht aufgegeben.\n\nBitte versuchen Sie es erneut oder wählen Sie eine andere Zahlungsmethode (Barzahlung / Karte).');
          }
