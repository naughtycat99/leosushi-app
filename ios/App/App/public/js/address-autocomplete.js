...
const AUTOCOMPLETE_REST_COORDS = { lat: 52.5659, lng: 13.3970 };
...
function initAddressAutocomplete() {
    ...
    async function fetchSuggestions(query) {
        try {
            ...
            const url = `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&lat=${AUTOCOMPLETE_REST_COORDS.lat}&lon=${AUTOCOMPLETE_REST_COORDS.lng}&limit=${SUGGESTIONS_LIMIT}&lang=de`;
            ...
            if (data && data.features && data.features.length > 0) {
                ...
                const localFeatures = data.features.map(feature => {
                    ...
                    const dist = haversineKm(
                        AUTOCOMPLETE_REST_COORDS.lat, AUTOCOMPLETE_REST_COORDS.lng,
    ...
    function renderSuggestions(features) {
        ...
        features.forEach(feature => {
            ...
            div.addEventListener('click', () => {
                const selectedStreet = street || name;
                const selectedHouseNumber = houseNumber || '';
                
                console.log('✅ Selected address:', selectedStreet, selectedHouseNumber, postcode, city);
                ...
                if (hnInput) {
                    hnInput.value = selectedHouseNumber;
                    // Trigger input event to update any validation/UI
